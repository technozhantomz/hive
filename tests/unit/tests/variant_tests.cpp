...
#define HIVE_TEST_VARIANT_OPERATOR_TEST( __OPERATOR__ )                                           \
do                                                                                                \
{                                                                                                 \
  using fc::variant;                                                                              \
  variant v;                                                                                      \
                                                                                                  \
  v = variant{ int64_t( -1 ) } __OPERATOR__ variant{ int64_t( 10 ) };                             \
  BOOST_REQUIRE( v.is_int64() );                                                                  \
  BOOST_REQUIRE_EQUAL( v.as_int64(), int64_t( -1 ) __OPERATOR__ int64_t( 10 ) );                  \
                                                                                                  \
  v = variant{ uint64_t( -1 ) } __OPERATOR__ variant{ uint64_t( 10 ) };                           \
  BOOST_REQUIRE( v.is_uint64() );                                                                 \
  BOOST_REQUIRE_EQUAL( v.as_uint64(), uint64_t( -1 ) __OPERATOR__ uint64_t( 10 ) );               \
                                                                                                  \
  v = variant{ double( 3.14 ) } __OPERATOR__ variant{ double( 3.141592 ) };                       \
  BOOST_REQUIRE( v.is_double() );                                                                 \
  BOOST_REQUIRE_EQUAL( v.as_double(), double( 3.14 ) __OPERATOR__ double( 3.141592 ) );           \
                                                                                                  \
  fc::variants vs1 = { { 1 }, { 2 }, { 3 } };                                                     \
  fc::variants vs2 = { { 4 }, { 5 }, { 6 } };                                                     \
                                                                                                  \
  v = variant{ vs1 } __OPERATOR__ variant{ vs2 };                                                 \
  BOOST_REQUIRE( v.is_array() );                                                                  \
  BOOST_REQUIRE_EQUAL( v.size(), vs1.size() );                                                    \
                                                                                                  \
  for( size_t i = 0; i < v.size(); ++i )                                                          \
  {                                                                                               \
    BOOST_REQUIRE( v[i].is_int64() );                                                             \
    BOOST_REQUIRE_EQUAL( v[i].as_int64(), vs1[i].as_int64() __OPERATOR__ vs2[i].as_int64() );     \
  }                                                                                               \
}                                                                                                 \
while(false)

HIVE_AUTO_TEST_CASE( operator_add,
  HIVE_TEST_VARIANT_OPERATOR_TEST( + );

  fc::variant v = fc::variant{ "alice" } + fc::variant{ "bob" };
  BOOST_REQUIRE( v.is_string() );
  BOOST_REQUIRE_EQUAL( v.as_string(), std::string{ "alice" } + std::string{ "bob" } );
)

HIVE_AUTO_TEST_CASE( operator_sub,
  HIVE_TEST_VARIANT_OPERATOR_TEST( - );
)

HIVE_AUTO_TEST_CASE( operator_mul,
  HIVE_TEST_VARIANT_OPERATOR_TEST( * );
)

HIVE_AUTO_TEST_CASE( operator_div,
  HIVE_TEST_VARIANT_OPERATOR_TEST( / );
)

...
def ref_node(request) -> RemoteNode:
  opt = request.config.getoption("--ref")
  return RemoteNode(opt) if opt else None

@fixture
def test_node(request) -> RemoteNode:
  opt = request.config.getoption("--test")
  return RemoteNode(opt) if opt else None
...
@fixture
def block_range(ref_node : RemoteNode):
  if ref_node is None:
    return range(0,0)
